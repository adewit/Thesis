...
in the final state, and how they are reconstructed in \ac{CMS}, will be discussed. Section \ref{sec:objects_pv} details the algorithms
...
made out of hits found in 2 or 3 layers of the inner tracker. Then, the track seeds
are extrapolated using a \ac{KF} \cite{trk-kf}, which extrapolates the seeding
trajectories along the expected flight path of a charged particle and adds 
additional matched hits it finds in the tracker layers it traverses to the track.
...
estimate of the track parameters. Finally, tracks that fail a specified set of quality 
...
for the position of the vertex using the associated tracks. The clustering 
...
vertex position. The analyses presented in CHAPMSSM take the primary
vertex of the hard scatter as the reconstructed vertex with largest scalar sum \pT~of
the associated tracks. This differs from the analysis presented in chapter \ref{chap:hhh},
...
described in section \ref{sec:objects_pf}.
Depending on the thickness of the intervening material, electrons lose up to 86 \% of their energy on
average by radiating photons before the \ac{ECAL} is reached.
To accurately reconstruct the energy of the electron it is
important to catch the energies of bremsstrahlung photons, which are spread in the $\phi$ direction
due to the magnetic field. Therefore ``supercluster'' algorithms are needed.
In these algorithms the clustering
is performed by defining a seed crystal as the crystal containg the most deposited energy
in a region under consideration, where the transverse energy in this crystal
passes a minimum threshold.
In the barrel, arrays of $5\times 1$ crystals in $\eta \times \phi$ are then added
to the cluster, stepping sideways in both directions in $\phi$. The array is
only added to the cluster if the energy deposited in it passes a threshold.
...
not perform optimally. Therefore a dedicated tracking algorithm is used.
...
Once the tracker seeds have been determined they are extrapolated
...
This BDT is trained using variables such as:
\begin{itemize}
\setlength{\itemsep}{-\baselineskip}
\item Cluster shape variables, such as $\sigma_{i\eta,i\eta}$, the energy-weighted width in units of $\eta$ crystals. 
\item Track quality variables
\item Distances in $\eta$ and $\phi$ betaween the supercluster and the associated track
\item Ratios between hadronic and electromagnetic energy in the region of the supercluster, between supercluster energy and momentum of the track, and including 
$1/E_{\text{SC}} - 1/p_{\Pe}$, the inverse of the supercluster energy, with the inverse of electron track momentum subtracted
...
is determined and if this is a distance between object i and an object j, objects
...
%\end{equation}
In this equation $\text{C}_{\text{offset}}$ is an offset correction, $\text{C}_{\text{MC}}$ an
...
As b--quarks appear in final states of many of the standard model 
processes being studied at the \ac{LHC}, as well as in searches for new
physics, it is important to correctly identify jets which originate
from the hadronisation of b--quarks.
As b--hadrons have a relatively
large mass of 5--6 GeV and a relatively long lifetime $\tau_B \approx 1.5$ ps \cite{pdg-2014},
they move for roughly $c\tau_B \approx 450$ $\mu$m before decaying. This means 
...
\begin{figure}
\begin{center}
\subfloat[CSVv2 distribution]{\includegraphics[width=0.5\textwidth]{./Objects/Plots/CSVDistributionMultijet.pdf}}
\subfloat[CSVv2 scale factors]{\includegraphics[width=0.5\textwidth]{./Objects/Plots/CSVSF.png}}
\end{center}
...
with $\Sigma \vec{p}_{\text{T}}^{\text{  lep}}$ corresponds to the vectorial sum of the transverse momenta of the
...
to the magnitude of the hadronic recoil. The BDT regression uses the magnitude of the hadronic
...
To correct for the mismodelling of \MET~in events without genuine \MET~ in \ac{MC} events, recoil corrections are determined. 
...
Figure \ref{fig:tau_iso_efficiency}a compares the \Pgt identification efficiency with the
...
The anti--electron discriminator is a BDT--based discriminator trained using input variables as:
\begin{itemize}
\setlength{\itemsep}{-\baselineskip}
...
Five working points are provided, ranging from very loose to very tight. The real tau efficiency
