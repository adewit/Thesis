...
In this chapter the reconstruction of the physics objects necessary to be able to perform searches with \Pgt leptons
in the final state in \ac{CMS} will be discussed. Section \ref{sec:objects_pv} details the algorithms
...
energy and hadronic taus as detailed in sections \ref{sec:objects_jets}, \ref{sec:objects_met} and \ref{sec:objects_tau} 
respectively.
...
made out of the hits found in 2 or 3 layers of the inner tracker. These track seeds
...
vertex position. The analyses presented in chapter \ref{chap:mssm} take the primary
vertex of the hard scatter as the reconstructed vertex for which the scalar sum of the \pT s of
...
The resolution of the x-- and z--coordinates of the primary vertex were measured in
...
primary vertices as a function of the scalar sum of the \pT s of the associated
tracks.]{Resolution of the x-- (a) and z--coordinate  (b)
of reconstructed primary vertices
as a function of the scalar sum of the \pT s of the associated tracks, 
measured in 2015 (blue triangles) and 2016 (red squares).
The resolution improves
with higher scalar sum of the \pT s of the associated tracks, which corresponds to larger numbers of tracks \cite{cms-trk-dp}.}
...
Depending on the thickness of the intervening material, electrons lose up to 86 \% of their energy on
average due to bremsstrahlung before reaching the \ac{ECAL}.
...
A dedicated tracking algorithm, with better performance than the \ac{CTF} for this 
particular case, is therefore used.
...
which extrapolates the position and energy of the supercluster to the first
layers of the tracker to determine the rough area where the tracker seeds should be.
...
has to be reconstructed as an electron by the \ac{PF} algorithm, 
...
decays. Both a cut--based and a BDT--based algorithm are available. For the selection of electrons in 
the analyses presented in later chapters the BDT--based algorithm is used, 
with exceptions for the loose identification criteria required for vetoing electrons. 
The variables used to train this BDT include:
\begin{itemize}
...
and misidentified electrons at 8 TeV, for data and MC]{$\sigma_{i\eta,i\eta}$ (a) and $1/E_{\text{SC}} - 1/p_{e}$ (b) for real (black points)  and 
misidentified (yellow triangles) electrons in a dataset corresponding to 19.7 fb$^{-1}$
...
of the electron, $\Delta R = 0.4$ during Run 1 and $\Delta R = 0.3$ during Run 2 \cite{CMS-PAS-HIG-16-037}.
...
this term; it is estimated as the sum of charged hadronic \pT~for hadrons not associated
with the primary vertex, multiplied by a $\Delta \beta$ factor of $\frac{1}{2}$.

\section{Muons}
\label{sec:objects_muo}
Muons are reconstructed by combining tracks in the inner tracker and tracks
...
tracker track, then re--applies the \ac{KF} on the combination
...
\begin{itemize}
\setlength{\itemsep}{-\baselineskip}
\item Reconstruction by either global or tracker muon reconstruction algorithms.
\item Requirements on track quality, and compatibility between the tracker-- and muon system track.
...
are made on the $\chi^2$/$N_{\text{dof}}$ of the track fit, number of hits of the inner
...
Just as described for electrons in section \ref{sec:objects_ele}, placing isolation 
...
energy \MET, defined in equation \ref{eqn:objects_met_corr}.
...
only a \ac{PF} photon is created. %Justified by fact that
...
Several methods exist for reducing the effects of pile--up particles 
...
including contributions from electronics noise and pile--up. This is
...
contributions from pile--up, electronics noise and the \ac{UE} of the hard interaction. The 
...
It is driven by uncertainties in the energy density used for pile--up
...
they move for roughly $c\tau \approx 450$ $\mu$m before decaying. This means
...
making use of the $\approx$ 11\% semileptonic branching fraction of b's. This
...
\caption[The b--tagging scale factors and errors for b--jets in the data sample collected during 2015 as a function of jet \pT~for the medium working point of the CSVv2 discriminant.]{%(a) distribution of CSVv2 discriminant for multijet events in 2.6 fb$^{-1}$ 
%collected at $\sqrt{s} = 13$ TeV and (b) 
The b--tagging scale factors and errors for b--jets in the data sample collected during 2015
...
Figure \ref{fig:objects_bjets} shows the b--tagging scale factors, and associated errors,
...
Therefore, to increase the accuracy of the \MET~measurement the jet energy corrections should be propagated as
...
The \MET~can also be mismeasured due to pile--up interactions, and an improved
...
chapter \ref{chap:mssm}. This algorithm computes a correction to the hadronic
...
constituents selected as the di--$\tau$ candidate.
...
\Zmm events, in a sample corresponding to 42 pb$^{-1}$ of data collected
at $\sqrt{s}=13$ TeV.]{(a) \MET~distributions comparing MVA \MET~(brown circles) to \ac{PF} \MET~ (black circles) for
\Zmm events, which contain no genuine \MET~, in a sample corresponding to 42 pb$^{-1}$ of data collected at $\sqrt{s}=$\mbox{13 TeV}. The \MET~
...
Two \ac{BDTs} trained on \Zmm events are used to compute this correction.
...
to the magnitude of the hadronic recoil. This BDT regression uses the magnitude of the hadronic
...
algorithm, an event--by--event
estimator of the likelihood that the \MET~ is due to unseen objects rather than induced
by mismeasurement of jet energy is also estimated \cite{cms-met-run1}.
...
To correct for the mismodelling of \MET~in \ac{MC} events without genuine \MET, recoil corrections are determined. 
...
The electron or photon with highest \pT~that is not yet included in a strip is used to build a new strip.
...
and strips are required to be within the signal cone $R_{\text{sig}} = \frac{3.0}{p_{\text{T}}[GeV]}$, always
...
\item \textbf{One prong + 1 \Ppizero :} One charged particle + one strip with $\Pgt$  mass requirement $ 0.3 < m_{\Pgt} < 1.3 \sqrt{p_{\text{T}}/100}$~GeV, with the
upper limit on the mass at least 1.3 and at most 4.2 GeV.
...
\item \textbf{One prong + 2 \Ppizero :} One charged particle + two strips. The $\Pgt$ mass should be $0.4 < m_{\Pgt} < 1.2\sqrt{p_{\text{T}}/100}$~GeV, with the
upper limit on the mass at least 1.2 and at most 4.0 GeV.
...
Requiring the reconstructed $\Pgt_{h}$ to be isolated reduces the jet $\rightarrow\Pgt_{h}$ fake rate. Isolation
...
of the identified hadronic tau. Only particles with \pT $> 0.5$ GeV within a cone of $\Delta R = 0.5$ centred around the 
direction of the hadronic tau are taken into account and the charged particle tracks are required to be compatible with having
...
In addition to the isolation sum, requirements on the sum of transverse momenta
...
\item Information about photons: the momentum weighted $\Delta R$ and/or $\Delta \eta$ and $\Delta \phi$ of photons in strips inside or outside the signal cone, in addition to the number of signal and isolation photons found.
\item $\Pgt_{h}$--related kinematic quantities, plus the reconstructed decay mode.  
...
Figure \ref{fig:tau_efficiency}a compares the $\Pgt_{h}$ identification efficiency with the
...
The anti--electron discriminator is a BDT--based discriminator trained using input variables such as:
\begin{itemize}
\setlength{\itemsep}{-\baselineskip}
\item Kinematic variables of the $\Pgt_{h}$ candidate and \ac{GSF} track, including visible mass of the $\Pgt_{h}$.
\item Energy/momentum ratios such as the fraction of electromagnetic energy, \ac{ECAL} and \ac{HCAL} energy relative to track momentum.
\item Photon--related variables, such as the momentum--weighted $\Delta \eta$ and $\Delta \phi$ between photons, the fraction of the $\Pgt_{h}$ energy carried by photons, and the number of photons found in strips.
...
Five working points are provided, ranging from very loose to very tight. The $\Pgt_{h}$ efficiency
...
\subfloat[Real $\Pgt_{h}$ efficiency for anti-electron discriminator]{\includegraphics[width=0.5\textwidth]{./Objects/Plots/TauEfficiencyEle.pdf}}
\subfloat[$\Pe\rightarrow\Pgt_{h}$ fake rate for the anti-electron discriminator]{\includegraphics[width=0.5\textwidth]{./Objects/Plots/TauMisIDEle.pdf}}
\end{center}
\caption[Figures of the jet$\rightarrow\Pgt_{h}$ fake rate in QCD MC events
...
working points of the anti-electron discriminator.]{(a) the jet$\rightarrow\Pgt_{h}$ fake rate in QCD \ac{MC} events versus $\Pgt_{h}$ identification
...
efficiency for different working points of the anti-electron discriminator, as a function of the \pT~of the $\Pgt_{h}$, and (c)
...
parts of the muon system around the direction of the $\Pgt_{h}$, and comparisons of calorimeter energy
with the momentum of the leading $\Pgt_{h}$ track.
%\begin{itemize}
%\setlength{\itemsep}{-\baselineskip}
