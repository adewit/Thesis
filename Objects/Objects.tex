...
all stable particles. This is of particular importance for the reconstruction of jets, missing transverse
...
vertex position. The analyses presented in chapter [MSSM] take the primary
vertex of the hard scatter as the reconstructed vertex for which the scalar sum \pT~of
...
fit and the distances between the vertex and the nominal interaction point 
...
with higher sum \pT~(corresponding to larger numbers of tracks) \cite{cms-trk-dp}.}
...
the inner tracker with energy deposits in the \ac{ECAL} \cite{cms-elereco-run1}.
...
important to capture the energies of bremsstrahlung photons, which are spread in the $\phi$ direction
due to the magnetic field. To achieve this, ``supercluster'' algorithms are used.
In these algorithms the clustering
is performed by defining a seed crystal as the crystal containg a local
...
superclusters if each cluster has a seed array with energy deposit above a minimum.
...
the change in electron track curvature can be large. % and so the \ac{CTF} does
...
by the use of an \ac{ECAL}-based track seeding algorithm
...
in later chapters a combined relative isolation variable is defined as \cite{SMHtautauCMS}:
...
For the analysis presented in chapter chapter [MSSM] ``medium'' muon requirements were applied \cite{CMS-PAS-HIG-16-037}.
These requirements include:
\begin{itemize}
\setlength{\itemsep}{-\baselineskip}
\item Reconstruction by either global or tracker muon reconstruction algorithms.
\item Requirements on track quality, and compatibility between tracker-- and muon system track.
\item Selection on the segment compatibility, a weight between 0 and 1 which is a measure of the compatibility
of a track with the muon track segments in muon stations it has crossed.
...
For the analysis presented in chapter [MSSM] a
...
for many analylses at the \ac{LHC} it is important to identify such jets correctly.
...
\ac{CSV}v2 version of the algorithm \cite{cms-btag-run2} used for the analysis presented in chapter [MSSM]. 
...
Therefore, the increase the accuracy of the \MET~measurement the jet energy corrections should be propagated as
...
chapter [MSSM]. This algorithm computes a correction to the hadronic
...
presented in chapters \ref{chap:hhh} and [MSSM] it is measured in \Zmm events, where no neutrinos
...
shows, 60\% of hadronic tau decays contain at least one \Ppizero in the final state. There is a large
...
of the identified hadronic tau. Only particles with \pT $> 0.5$ GeV within a cone $\Delta R = 0.5$ centred around the 
...
\subfloat[Anti-e discriminator efficiency]{\includegraphics[width=0.5\textwidth]{./Objects/Plots/TauEfficiencyEle.pdf}}
\subfloat[Anti-e discriminator fake rate]{\includegraphics[width=0.5\textwidth]{./Objects/Plots/TauMisIDEle.pdf}}
\end{center}
