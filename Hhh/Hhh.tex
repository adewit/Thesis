...
For these results I contributed to the analysis of the \etau and \mutau channels, studying alternative \ttbar
background rejection methods and other uses of the kinmeatic fit variable which are not further discussed.
My main contribution was the statistical inference for the \Htohh analysis, which included limit setting and performing the model interpretations
...
Signal and background events are generated using several different \ac{MC} event generators. With the exception
of the embedded samples described later in this section, all of these simulated
samples are centrally produced for use by all CMS analyses. The \texttt{MadGraph}
...
are used for the \Wjets and \Zellell backgrounds. This increases the number of simulated background events
...
To better model the \Ztautau background an embedding technique is used \cite{SMHtautauCMS}. In this method, 
...
the isolation sum (equation \ref{eqn:cutbased_iso}) is required to be
...
\begin{figure}[h!]
\begin{center}
\subfloat[\mutau channel]{\includegraphics[width=0.5\textwidth]{Hhh/Plots/mt_1_2jetinclusive_mt_2012_corrs.pdf}}
\subfloat[\etau channel]{\includegraphics[width=0.5\textwidth]{Hhh/Plots/mt_1_2jetinclusive_et_2012_corrs.pdf}}
\end{center}
\caption[\mT~distributions in the \mutau and \etau channels.]{\mT~distributions in the (a) \mutau and (b) \etau channels. The signal
at \mH~$=300\,\GeV$ and \tanb~$=2$ in the \ac{MSSM} low-\tanb~scenario, multiplied by a factor of 100, is overlaid.}
...
events, as the \PW boson is very heavy, the lepton and neutrino are more likely to be emitted back-to-back, and
...
Jets are considered b-tagged if they pass the medium working point of the \ac{CSV} b-tagging discriminator. The
typical b-tagging efficiency at this working point is 70\% \cite{BTV8TeV}.
The 2jet-0tag category does not collect much of the signal and is dominated by 
backgrounds, and the 2jet-2tag category is the most sensitive to signal. 
...
\begin{figure}[h!]
\begin{center}
\subfloat[\mutau channel]{\includegraphics[width=0.5\textwidth]{Hhh/Plots/n_bjets_csv_2jetinclusive_mt_2012_log_corrs.pdf}}
\subfloat[\etau channel]{\includegraphics[width=0.5\textwidth]{Hhh/Plots/n_bjets_csv_2jetinclusive_et_2012_log_corrs.pdf}}
\end{center}
...
are the decay products of a $125\,\GeV$ Higgs boson, and therefore their invariant
...
\begin{figure}[h!]
\begin{center}
\subfloat[$m_{\Pgt\Pgt}$]{\includegraphics[width=0.5\textwidth]{Hhh/Plots/m_sv_2jet2tag_mt_2012_corrs.pdf}}
\subfloat[$m_{jj}$ ]{\includegraphics[width=0.5\textwidth]{Hhh/Plots/mjj_2jet2tag_mt_2012_corrs.pdf}}
\end{center}
...
b-/c-jets and light jets. 

In this analysis the ``promote-demote'' method
is used to apply these scale factors. In this method a demotion 
or promotion probability is assigned to every
jet as:
...
$m(\text{j}_{1},\text{j}_{2}) = m_{\PHiggslight} = 125\,\GeV$.
...
For jets, the reconstructed direction in $\eta$ and $\phi$ is 
...
uncertainties in the momentum measurement, and therefore $\vec{\beta} = \vec{p}/E$ is
also kept constant. 

Before the fit, the mass constraint gives:
~\vspace{-0.5\baselineskip}
\begin{align}\label{eqn:kinfit_prefit}
m_{\PHiggslight}^2 &= E_{\PHiggslight}^2 - \vec{p_{\PHiggslight}}^2 = (E_{\text{j} 1}+E_{\text{j} 2})^2 - (\vec{p_{\text{j} 1}} + \vec{p_{\text{j} 2}})^2 \notag\\
& = 2E_{\text{j} 1}E_{\text{j} 2} + m_{\text{j} 1}^2 +m_{\text{j} 2}^2 - 2\vec{p_{\text{j} 1}}\vec{p_{\text{j} 2}} \notag\\
& = m_{\text{j} 1}^2 +m_{\text{j} 2}^2 - 2E_{\text{j} 1}E_{\text{j} 2}(1-\vec{\beta_{\text{j} 1}}\vec{\beta_{\text{j} 2}})\notag\\
&\Rightarrow (1-\vec{\beta_{\text{j} 1}}\vec{\beta_{\text{j} 2}}) = \frac{m_{\PHiggslight}^2 - m_{\text{j} 1}^2 - m_{\text{j} 2}^2}{2E_{\text{j} 1}E_{\text{j} 2}}.
\end{align}
When varying the energy of the first jet in the fit, equation \ref{eqn:kinfit_prefit} can be 
rewritten to derive an expression for the updated energy of the second jet. Using $\mathcal{A} = (1-\vec{\beta_{\text{j} 1}}\vec{\beta_{\text{j} 2}})$ 
and
~\vspace{-0.5\baselineskip}
...
m_{\PHiggslight}^2 &= m_{\text{j} 1}^2 + m_{\text{j} 2}^2 - 2E_{\text{j} 1}E_{\text{j} 2}(1-\vec{\beta_{\text{j} 1}}\vec{\beta_{\text{j} 2}})\notag\\
&= m_{\text{j} 1}^2 +\frac{E_{\text{j} 2,\text{fit}}^2}{\gamma_{\text{j} 2}^2} +2\mathcal{A}E_{\text{j} 1}E_{\text{j} 2,\text{fit}}\notag\\
\Rightarrow E_{\text{j} 2}^{\text{fit}}& = \frac{(-2E_{\text{j} 1}\mathcal{A} + \sqrt{4E_{\text{j} 1}^2\mathcal{A} - 4(m_{\text{j} 1}^2-m_{\PHiggslight}^2)\gamma_{\text{j} 2}^{-2}})\gamma_{\text{j} 2}^2}{2}\notag\\
&= - E_{\text{j} 1}\mathcal{A}\gamma_{\text{j} 2}^2+\sqrt{E_{\text{j} 1}^2\mathcal{A}\gamma_{\text{j} 2}^4 - (m_{\text{j} 1}^2 - m_{\PHiggslight}^2)\gamma_{\text{j} 2}^2}\notag\\
&\Rightarrow E_{\text{j} 2}^{\text{fit}} = E_{\text{j} 1}\mathcal{A}\gamma_{\text{j} 2}^2(-1 + \sqrt{1+\frac{m_{\PHiggslight}^2 - m_{\text{j} 1}^2}{E_{\text{j} 1}^2\mathcal{A}\gamma_{\text{j} 2}^2}}).
\end{align}

The fitting procedure additionally modifies the masses of the jets as 
~\vspace{-0.5\baselineskip}
\begin{equation}\label{eqn:kinfit_bjetmass}
m_{\text{j}_{1,2}}^{\text{fit}} = m_{\text{j}{1,2}}\frac{E_{\text{j}{1,2}}^{\text{fit}}}{E_{\text{j}{1,2}}}.
\end{equation}
The $\chi^2$ term for each jet is calculated as $\chi_{\text{j} 1,2}^2 = \frac{E_{\text{j} 1,2}^{\text{fit}}-E_{\text{j} 1,2}^{\text{meas}}}{\sigma_{\text{j} 1,2}}$, where `meas' denotes the measured value and `fit' the value given by the fit.

For taus only the visible decay products are measured. Assuming these point in 
the same direction as the original tau, a similar procedure as for the jets can be employed to constrain the energy of the second tau
...
\vec{p}_{\text{T,recoil}}^{\text{ meas}} = -\vec{p}_{\text{T,\PHiggs}}^{\text{ meas}} = -\vec{p}_{\text{T,miss}}^{\text{ meas}} - \vec{p}_{\text{T},\text{j} 1}^{\text{ meas}} - \vec{p}_{\text{T},\text{j} 2}^{\text{ meas}} - \vec{p}_{\text{T},\Pgt_1^{\text{vis}}}^{\text{ meas}} - \vec{p}_{\text{T},\Pgt_2^{\text{vis}}}^{\text{ meas}},
\end{equation}
and the recoil from the fit is,
~\vspace{-0.5\baselineskip}
\begin{equation}\label{eqn:kinfit_recoilfit}
\vec{p}_{\text{T,recoil}}^{\text{ fit}} = -\vec{p}_{\text{T,\PHiggs}}^{\text{ fit}} = - \vec{p}_{\text{T},\text{j} 1}^{\text{ fit}} - \vec{p}_{\text{T},\text{j} 2}^{\text{ fit}} - \vec{p}_{\text{T},\Pgt_1}^{\text{ fit}} - \vec{p}_{\text{T},\Pgt_2}^{\text{ fit}}.
\end{equation}
...
\chi^2_{\text{recoil}} = (\vec{p}_{\text{T,recoil}}^{\text{ fit}} - \vec{p}_{\text{T,recoil}}^{\text{ meas}})^T V_{\text{recoil}}^{-1}(\vec{p}_{\text{T,recoil}}^{\text{ fit}} - \vec{p}_{\text{T,recoil}}^{\text{ meas}}),
\end{equation}
where the covariance matrix $V_{\text{recoil}}$ of the recoil vector is estimated as 
\begin{equation}\label{eqn:kinfit_recoilcov}
V_{\text{recoil}} = V_{\vec{p}_{\text{T,miss}}} - V_{\text{j} {1}} - V_{\text{j} 2},
\end{equation}
 from the covariance matrices of the jets and
...
while the covariance matrices of the jets can be written in terms of the position, momentum, and energy resolution of the jets. 
The total $\chi^2$ term that needs to be minimised in the fit can then be written as
~\vspace{-0.5\baselineskip}
\begin{equation}\label{eqn:kinfit_chitot}
\chi^2 = \chi^2_{\text{j} 1} + \chi^2_{\text{j} 2} + \chi^2_{\text{recoil}}.
\end{equation}
The results of the kinematic fit are the tau and jet four-vectors with their energies and momenta
...
shown in figure \ref{fig:Hhh_selection_masscuts}. This comparison between four-body mass and
...
described in section \ref{sec:hhh_datasets}. The yield of
\Ztautau in each category is evaluated in two steps. In the first
step the total yield after the initial event selection, before subdividing into categories,
is evaluated in the simulated \Ztautau sample. In the second step the yield obtained 
in the first step is scaled by the efficiency with which such events pass the 
category selection in the embedded sample.
The shape of
...
The \ttbar shape and normalisation are both estimated using simulation.
...
The resulting scale factor is $1.033 \pm 0.013\text{ (stat.)}\pm 0.088\text{ (syst.)}$. The
statistical and systematic uncertainties are added in quadrature to give a 9\% uncertainty,
which is applied to the \ttbar process.
...
estimated using simulated samples. Dedicated $\Pe\rightarrow\Pgth$
fake rate corrections, described in section \ref{sec:hhh_datamc}, are
applied to adjust the \Zee normalisation. The $\Pgm\rightarrow\Pgth$ 
fake rate correction was found to be close to 1 and therefore the \Zmm
normalisation is not subject to such an additional correction factor.
...
shape of the kinematic fit mass distribution. 
...
fake rate correction is close \mbox{to 1,} which is why it is not applied, but the uncertainty is 
taken into account.
...
\item \ttbar: The uncertainty in the measured \ttbar scale factor 9\%. 
...
The first factor in equation \ref{eqn:hhh_likelihood} runs over all bins of the $m_{\PHiggs}^{\text{kinfit}}$ distribution 
...
The second factor in equation \ref{eqn:hhh_likelihood}, $p(\tilde{\theta}|\theta)$, represents the full set of 
...
1-p_b = P(q_{\mu} \geq q_{\mu}^{\text{obs}} | \text{b}) = \int_{q_{\mu}^{\text{obs}}}^{\infty} f(q_{\mu} | 0,\hat{\theta_{0}^{\text{obs}}}) dq_{\mu} ,
\end{equation}
...
respectively. The numbers of observed events, compared with the background expectation, are given in appendix \ref{appendix:yields_bbtautau}.
