...
parser.add_argument(
    ...
    contours""")
parser.add_argument(
    '--extra_contour_names', default=None, help="""Names of contours to be
superimposed""")
parser.add_argument(
...
if args.extra_contour_file is not None:
    ...
    for filename in contour_files:
        ...
        if args.extra_contour_names is not None:
            extra_contour_names = args.extra_contour_names.split(',')
...
for c in types:
    ...
    for x in xrange(1, hists[c].GetNbinsX()+1):
        for y in xrange(1, hists[c].GetNbinsY()+1):
            xvar = hists[c].GetXaxis().GetBinCenter(x)
            yvar = hists[c].GetYaxis().GetBinCenter(y)
            if xvar > 450 and xvar < 700 and yvar > 54:
                hists[c].SetBinContent(x, y, 0.)

    contours[c] = plot.contourFromTH2(hists[c], CL, 5, frameValue=1)
    if debug is not None:
        hists[c].SetName('hist_%s'%c)
        debug.WriteTObject(hists[c], 'hist_%s' % c)
        for i, cont in enumerate(contours[c]):
            cont.SetName('cont_%s_%i'%(c,i))
...
#    for gr in extra_contours:#[i]:
        plot.Set(extra_contours[i],LineWidth=2,LineColor=ROOT.kBlue,LineStyle=int(contour_styles[i]))
        extra_contours[i].Draw('LSAME')
...
plot.Set(legend, NColumns=3, Header='#bf{%.0f%% CL Excluded:}' % (args.CL*100.))
if 'obs' in contours:
...
#exp-2 and exp0 swapped for hMSSM
if 'exp0' in contours:
    if 'obs' in contours:
...
if extra_contours is not None:
    ...
    for i in range(0,len(contour_title)): 
        legend.AddEntry(extra_contours[i],contour_title[i],"L")
