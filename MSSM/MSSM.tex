...
The basis of the event selection is to select \mutau, \etau,
...
most of the signal samples at high-mass, the region of interest in this
...
hadronic taus. Such a selection is driven by the $\eta$ range covered by the tracker and the $\eta$
range that is used in the reconstruction of the particles,
although in some cases the used trigger is restricted in $\eta$ and the cut is chosen to
align with that. As an illustration figure \ref{fig:mt_muonpt}b shows
...
ensure that the particle is compatible with having
originated from the primary vertex. This reduces the selection of particles
coming from pile--up events. Additional identification and isolation criteria
are also applied. These will be discussed in the following sections.
The optimisation of some of the selections will also be discussed there. Note that 
the optimisations were all based on the dataset collected during 2015, which
...
taus are concerned, for if there were additional electrons
or muons the event would not pass the electron or muon veto.
...
\begin{figure}[h!]
\begin{center}
\subfloat[gg$\phi$]{\includegraphics[width=0.5\textwidth]{./MSSM/Figures/mssm_thesis_ttopt_ggH.pdf}}
\subfloat[bb$\phi$]{\includegraphics[width=0.5\textwidth]{./MSSM/Figures/mssm_thesis_ttopt_bbH.pdf}}
\end{center}
...
\begin{figure}[h!]
\begin{center}
\subfloat[\Zmm events]{\includegraphics[width=0.5\textwidth]{./MSSM/Figures/pt_tt_inclusive_zmm_2016_log_noweight.png}}
...
\ref{fig:mttot_sigseps}, where figure \ref{fig:mttot_sigseps} shows the total transverse mass
...
it has two real taus in the final state, like the signal that we are searching for. The only aspect
that distinguishes the \Ztautau background from the gluon fusion signal is the mass of the particle. Smaller backgrounds
from \Zellell decays also play a role, especially in the \mutau and \etau channels where such events
might pass the selection for example if a jet is misreconstructed as a hadronic tau and only one of the leptons is reconstructed. 
...
This is because top quarks virtually always decay as $\Ptop\rightarrow \PW\Pbottom$. and so such events 
always contain real b-jets. One or both of the \PW bosons, and additional jets in the event, can 
then form \etau, \mutau, \tautau and \emu pairs.
The \Wjets background arises in the \etau and \mutau channels when the lepton from the \PW decay and a jet misreconstructed
...
In the \emu channel an additional lepton would have to originate from one of the jets, and so here this
background is smaller. Backgrounds that are small in all channels are the di-boson and single-top backgrounds. In 
...
in the estimation of QCD and \Wjets in the \etau and \mutau channels
are included in a simultaneous fit with the signal region to obtain
the final results. More detail on this is given in section ADD SECTION.
In the \emu and \tautau channels the \Wjets background is estimated using
\ac{MC} samples. The \Ztautau and \Zellell backgrounds are also estimated
using \ac{MC} samples, and a \Zmm control region
