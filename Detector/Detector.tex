...
The energy resolution of the \ac{ECAL} can be parameterised as in
...
the \ac{CSC} covering the $1.2<|\eta|<2.4$ region. The \ac{RPC}, covering the same
regions as both \ac{DT} and \ac{CSC}, is used as a dedicated triggering system.

A cross-section of one quadrant of the detector, indicating
the positions of the \ac{DT},\ac{CSC} and \ac{RPC}, is shown in figure \ref{fig:CMS_MuonSystem}. 
\begin{figure}[h!]
\begin{center}
...
The \ac{DT} consists of 

%UPGRADE
During LS1, the endcaps of the muon system were upgraded. Each endcap received an additional muon station, extending ME/4 (figure \ref{fig:CMS_MuonSystem}) out towards
$R\approx 700 $cm, in line with the other three muon stations \cite{cms-muon-upgrade}.
...
replaced by a completely upgraded system, which provides amongst others 
...
trigger. In this step basic object identification is performed.

\begin{figure}[h!]
\begin{center}
\includegraphics[width=0.5\textwidth]{./Detector/Plots/CMSTrigger.png}
\caption{Schematic of the different components of the \ac{L1} trigger \cite{cms-trigger-upgrade}.}
